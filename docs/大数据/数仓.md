...
* dwd_fact_order_info 用于上表的过滤条件，主要用支付时间过滤支付订单。
* dwd_fact_refund_info 统计退款
* dwd_fact_cart_info 
* dwd_fact_favor_info 
...
| wk_dt | string | 持续时间 |
| continuity_count | bigint | 活跃用户统计 |
...
| dt | string | 统计日期 |
| sku_id | string |商品ID  |
| payment_amount |  decimal(10,2)  | 销量 |
...
##### 商品加入购物车排名（ads_product_cart_topN）
...
| order_amount | decimal(10,2)  | 单日下单金额 |
...
| ---- | ---- | ---- |
| dt | string | 统计日期 |
| payment_count | bigint | 单日支付笔数 |
| payment_amount | decimal(10,2) | 单日支付金额 |
